...
// Module represents the Role feature module.
type Module struct {
	handler    *Handler
...
// ModuleConfig holds dependencies for the role module.
...
// NewModule creates and wires the role module.
func NewModule(cfg ModuleConfig) (*Module, error) {
	logger := cfg.Logger.WithName("role")
...
func (m *Module) subscribeToEvents() {
	...
	// React to database creation - roles can now be granted access
	m.eventBus.Subscribe(eventbus.EventDatabaseCreated, "role.OnDatabaseCreated",
	...
	// React to database deletion - role grants may need cleanup
	m.eventBus.Subscribe(eventbus.EventDatabaseDeleted, "role.OnDatabaseDeleted",
		func(ctx context.Context, event eventbus.Event) error {
	...
	// React to instance disconnection - role operations may fail
	m.eventBus.Subscribe(eventbus.EventInstanceDisconnected, "role.OnInstanceDisconnected",
		func(ctx context.Context, event eventbus.Event) error {
			instEvent, ok := event.(*eventbus.InstanceDisconnected)
			if !ok {
				return nil
			}
			m.logger.V(1).Info("Instance disconnected, role operations may fail",
