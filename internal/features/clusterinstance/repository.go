...
// Repository handles cluster instance operations via the service layer.
...
// NewRepository creates a new cluster instance repository.
...
func (r *Repository) withService(ctx context.Context, instance *dbopsv1alpha1.ClusterDatabaseInstance, fn func(svc *service.InstanceService) error) error {
	// Determine the namespace for credentials
	// For cluster-scoped resources, the secret must specify a namespace
	credNamespace := ""
	if instance.Spec.Connection.SecretRef != nil {
		credNamespace = instance.Spec.Connection.SecretRef.Namespace
		if credNamespace == "" {
			return fmt.Errorf("cluster-scoped ClusterDatabaseInstance requires secretRef.namespace to be specified")
		}
	}

	// Get credentials
	creds, err := r.secretManager.GetCredentials(ctx, credNamespace, instance.Spec.Connection.SecretRef)
	if err != nil {
	...
	if instance.Spec.TLS != nil && instance.Spec.TLS.Enabled {
		tlsCreds, err := r.secretManager.GetTLSCredentials(ctx, credNamespace, instance.Spec.TLS)
		if err != nil {
...
func (r *Repository) Connect(ctx context.Context, instance *dbopsv1alpha1.ClusterDatabaseInstance) (*ConnectResult, error) {
...
func (r *Repository) Ping(ctx context.Context, instance *dbopsv1alpha1.ClusterDatabaseInstance) error {
...
func (r *Repository) GetVersion(ctx context.Context, instance *dbopsv1alpha1.ClusterDatabaseInstance) (string, error) {
	var version string
...
// GetInstance retrieves a ClusterDatabaseInstance by name.
// Note: ClusterDatabaseInstance is cluster-scoped, so no namespace is needed.
func (r *Repository) GetInstance(ctx context.Context, name string) (*dbopsv1alpha1.ClusterDatabaseInstance, error) {
