...
// Module represents the ClusterRole feature module.
type Module struct {
	handler    *Handler
...
func NewModule(cfg ModuleConfig) (*Module, error) {
	logger := cfg.Logger.WithName("clusterrole")
...
func (m *Module) subscribeToEvents() {
	...
	// React to cluster instance disconnection - cluster role operations may fail
	m.eventBus.Subscribe(eventbus.EventInstanceDisconnected, "clusterrole.OnInstanceDisconnected",
		func(ctx context.Context, event eventbus.Event) error {
			instEvent, ok := event.(*eventbus.InstanceDisconnected)
			if !ok {
				return nil
			}
			m.logger.V(1).Info("Instance disconnected, cluster role operations may fail",
