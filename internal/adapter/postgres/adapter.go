...
// Adapter implements the DatabaseAdapter interface for PostgreSQL
type Adapter struct {
...
func (a *Adapter) GetVersion(ctx context.Context) (string, error) {
	...
	var version string
	err := a.pool.QueryRow(ctx, "SHOW server_version").Scan(&version)
	if err != nil {
...
func (a *Adapter) execWithNewConnection(ctx context.Context, database, query string, args ...interface{}) error {
	...
	connString := fmt.Sprintf(
		...
		config.Host, config.Port, database, config.Username, config.Password,
		func() string {
			if config.SSLMode != "" {
...
// escapeIdentifier escapes a PostgreSQL identifier (table, column, etc.)
...
// escapeLiteral escapes a PostgreSQL string literal
