...
var _ = Describe("CockroachDB Adapter", func() {
	...
	Describe("buildConnectionString", func() {
		It("should build basic connection string with CockroachDB defaults", func() {
			config := testutil.NewBasicConnectionConfig()
			adapter := NewAdapter(config)
			...
			Expect(connStr).To(ContainSubstring("port=26257"))
			Expect(connStr).To(ContainSubstring("dbname=defaultdb"))
			Expect(connStr).To(ContainSubstring("user=root"))
	...
	Describe("escapeIdentifier", func() {
		It("should quote identifiers", func() {
			Expect(escapeIdentifier("my_table")).To(Equal(`"my_table"`))
		})

		It("should double existing quotes", func() {
			Expect(escapeIdentifier(`my"table`)).To(Equal(`"my""table"`))
		})

		It("should handle empty string", func() {
			Expect(escapeIdentifier("")).To(Equal(`""`))
		})

		It("should handle multiple quotes", func() {
			Expect(escapeIdentifier(`a"b"c`)).To(Equal(`"a""b""c"`))
		})
		...
		It("should handle special characters", func() {
			Expect(escapeIdentifier("table$name")).To(Equal(`"table$name"`))
		})
	})

	Describe("escapeLiteral", func() {
		It("should quote literals", func() {
			Expect(escapeLiteral("my_value")).To(Equal(`'my_value'`))
		})

		It("should double existing single quotes", func() {
			Expect(escapeLiteral(`my'value`)).To(Equal(`'my''value'`))
		})

		It("should handle empty string", func() {
			Expect(escapeLiteral("")).To(Equal(`''`))
		})

		It("should handle multiple single quotes", func() {
			Expect(escapeLiteral(`a'b'c`)).To(Equal(`'a''b''c'`))
		})
	...
	Describe("Connection Config Variations", func() {
		It("should handle config with all fields set", func() {
			...
			Expect(err).NotTo(HaveOccurred())
			Expect(connStr).To(ContainSubstring("host=crdb.example.com"))
			Expect(connStr).To(ContainSubstring("port=26257"))
