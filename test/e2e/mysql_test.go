...
var _ = Describe("mysql", Ordered, func() {
	...
	Context("Database Ownership Verification", func() {
		It("should identify database owner (user with full privileges)", func() {
			By("querying database owner from MySQL")
			// MySQL doesn't have explicit database ownership like PostgreSQL
			...
			// In MySQL, the database is created by root, so it has full privileges
			GinkgoWriter.Printf("Database '%s' owner/admin: %s\n", databaseName, owner)
			// Owner should be either root (default) or the user we granted ALL to
