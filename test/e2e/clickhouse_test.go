...
var _ = Describe("clickhouse", Ordered, func() {
	...
	Context("DatabaseGrant lifecycle", func() {
		...
		It("should create a DatabaseGrant and become Ready", func() {
			By("creating a DatabaseGrant CR with ClickHouse-specific grant config")
						...
						"clickhouse": map[string]interface{}{
							"grants": []interface{}{
								map[string]interface{}{
									...
									"privileges": []interface{}{"SELECT", "INSERT", "CREATE"},
								},
							},
						},
					},
			...
			By("verifying user has SELECT privilege on the database")
			Eventually(func() bool {
	...
	Context("Grant Enforcement Verification", func() {
		...
		It("should enforce granted permissions (SELECT, INSERT allowed; DELETE denied)", func() {
			...
			// Grant SELECT, INSERT on the table to the test user
			Eventually(func() error {
			...
			adminConn2, _ := verifier.ConnectAsUser(ctx, adminUsername, adminPassword, databaseName)
			if adminConn2 != nil {
				_ = adminConn2.CanDropTable(ctx, testTable)
				adminConn2.Close()
			}
		})
	})
	...
	Context("Role Membership Verification", func() {
		...
		It("should verify role membership after assignment", func() {
			...
			deleteAndWait(ctx, databaseGrantGVR, testNamespace, memberUser+"-role-grant", timeout, interval)
