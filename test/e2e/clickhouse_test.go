...
var _ = Describe("clickhouse", Ordered, func() {
	const (
		...
		clickhouseHost  = "host.k3d.internal" // Docker host from k3d (ClickHouse runs in Docker Compose)
		secretName      = "clickhouse-credentials"
		secretNamespace = "clickhouse"
		timeout         = 2 * time.Minute
		interval        = 2 * time.Second
		defaultCHPort   = int32(9000) // native protocol port
	)

	ctx := context.Background()

	// Database verifier for validating actual database state
	var verifier *testutil.ClickHouseVerifier
	...
	// The E2E mapped port is 19000; defaultCHPort (9000) is used as the fallback.
	...
	BeforeAll(func() {
		By("setting up ClickHouse verifier")
		...
		Expect(err).NotTo(HaveOccurred(), "Failed to get ClickHouse username from secret")

		adminPassword, err = getSecretValue(ctx, secretNamespace, secretName, "password")
		Expect(err).NotTo(HaveOccurred(), "Failed to get ClickHouse password from secret")
		...
		cfg := testutil.ClickHouseEngineConfig(verifierHost, verifierPort, adminUsername, adminPassword)
		verifier = testutil.NewClickHouseVerifier(cfg)

		// Connect with retry since database may still be starting
		Eventually(func() error {
			return verifier.Connect(ctx)
		}, timeout, interval).Should(Succeed(), "Should connect to ClickHouse for verification")
	})
				...
				Object: map[string]interface{}{
					...
					"spec": map[string]interface{}{
						...
						"connection": map[string]interface{}{
							...
							"database": "default", // ClickHouse default database
	...
	Context("Database lifecycle", func() {
		It("should create a Database and become Ready", func() {
			...
			By("verifying database actually exists in ClickHouse")
			Eventually(func() bool {
				...
				return exists
			}, timeout, interval).Should(BeTrue(), "Database '%s' should exist in ClickHouse", databaseName)
		})
	})
	...
	Context("DatabaseUser lifecycle", func() {
		It("should create a DatabaseUser with generated secret", func() {
			...
			By("verifying user actually exists in ClickHouse")
			Eventually(func() bool {
				...
				return exists
			}, timeout, interval).Should(BeTrue(), "User '%s' should exist in ClickHouse", userName)
		})
	})
	...
	Context("Cross-namespace Secret reference", func() {
		It("should support cross-namespace Secret reference (ClusterRoleBinding test)", func() {
			...
			Expect(secretRef["namespace"]).To(Equal(secretNamespace), "Secret should be in clickhouse namespace")
	...
	Context("DatabaseRole lifecycle", func() {
		...
		It("should create a DatabaseRole and become Ready", func() {
			...
			By("verifying role actually exists in ClickHouse")
			Eventually(func() bool {
				...
				return exists
			}, timeout, interval).Should(BeTrue(), "Role '%s' should exist in ClickHouse", roleName)
		})
	})
	...
	Context("DatabaseGrant lifecycle", func() {
		...
		It("should create a DatabaseGrant and become Ready", func() {
			...
			// ClickHouse uses clickhouse-specific grant syntax with database-level privileges
			...
			By("verifying user has SELECT privilege on database in ClickHouse")
			Eventually(func() bool {
				hasPriv, err := verifier.HasPrivilegeOnDatabase(ctx, userName, "SELECT", "", "", databaseName)
				if err != nil {
	...
	Context("Grant Enforcement Verification", func() {
		...
		It("should enforce granted permissions (SELECT, INSERT allowed; ALTER DELETE denied)", func() {
			...
			// Create test table using ClickHouse MergeTree engine
			...
			// ClickHouse uses: GRANT <privileges> ON <database>.<table> TO <user>
			Eventually(func() error {
				...
				return retryConn.Exec(ctx, "GRANT SELECT, INSERT ON "+databaseName+"."+testTable+" TO "+userName)
			...
			By("verifying ALTER TABLE DELETE is denied (ALTER privilege not granted)")
			// ClickHouse uses ALTER TABLE ... DELETE for row deletions (lightweight mutations).
			// Without the ALTER privilege, this should fail.
			err = userConn.CanDeleteData(ctx, testTable)
			Expect(err).To(HaveOccurred(), "ALTER TABLE DELETE should be denied without ALTER privilege")
			GinkgoWriter.Printf("ALTER TABLE DELETE correctly denied: %v\n", err)

			By("cleaning up test table")
			adminCleanup, _ := verifier.ConnectAsUser(ctx, adminUsername, adminPassword, databaseName)
			if adminCleanup != nil {
				_ = adminCleanup.CanDropTable(ctx, testTable)
				adminCleanup.Close()
			}
		})
	})
	...
	Context("Drift Correction", func() {
		It("should recreate database after manual drop", func() {
			...
			Expect(exists).To(BeTrue(), "Database '%s' should exist before drift", databaseName)

			By("manually dropping the database to simulate drift")
			adminConn, err := verifier.ConnectAsUser(ctx, adminUsername, adminPassword, "default")
			Expect(err).NotTo(HaveOccurred(), "Should connect as admin to default database")
			defer adminConn.Close()

			err = adminConn.Exec(ctx, "DROP DATABASE IF EXISTS "+databaseName)
			Expect(err).NotTo(HaveOccurred(), "Admin should be able to drop the database")

			By("verifying database is gone")
			Eventually(func() bool {
				exists, err := verifier.DatabaseExists(ctx, databaseName)
				if err != nil {
					return false
				...
				return !exists
			}, timeout, interval).Should(BeTrue(), "Database should be gone after manual drop")

			By("waiting for operator to detect drift and recreate the database")
			Eventually(func() bool {
				exists, err := verifier.DatabaseExists(ctx, databaseName)
				if err != nil {
					GinkgoWriter.Printf("Error checking database existence: %v\n", err)
				...
				return exists
			}, timeout, interval).Should(BeTrue(), "Operator should recreate database '%s' after drift", databaseName)

			GinkgoWriter.Printf("Database '%s' successfully recreated by operator after drift\n", databaseName)
		})
	})

	Context("Negative tests", func() {
		var negativeTestConfig *DatabaseTestConfig

		BeforeEach(func() {
			negativeTestConfig = &DatabaseTestConfig{
		...
		It("should fail gracefully with invalid instance credentials", func() {
			negativeTestConfig.RunInvalidInstanceConnectionTest(ctx)
		})

		It("should fail or stay pending with non-existent instanceRef", func() {
			negativeTestConfig.RunNonExistentInstanceRefTest(ctx)
		})

		It("should block deletion when deletion protection is enabled", func() {
			negativeTestConfig.RunDeletionProtectionTest(ctx)
		})
	})
